...
class Builder:
    ...
    def build(self) -> Dict[str, Any]:
        ...
        # 数据性质。**这一条必须跟着注册表走，一路带到模型看到的 meta 里。**
        #
        # 2026-08-28 踩到的：桩数据后端的 meta 里有一句「非真实业务数值，不可用于
        # 对外结论」，实测模型每次都会主动向用户声明。切到 starrocks 后端之后那句话
        # **静默消失了** —— 而当时连的是 dev 复刻库，数值仍然是造的。模型于是对着
        # 一堆假数写出了「周末积累、周初释放」「疑似运营活动消退」这样的业务分析。
        #
        # 和「加一层前置检查让后面几层的验证静默失效」是同一个形状：
        # **换掉一个组件，会让挂在旧组件上的免责声明一起消失，而没有任何东西会报错。**
        # 所以声明不能挂在后端上，要挂在数据本身（注册表）上。
        notice = None
        if self.schema != "ads":
            notice = (f"⚠️ 本次数据来自 `{self.schema}`，**不是生产 ads 层**。"
                      f"这是开发用的复刻库，表结构和口径注释复制自生产，"
                      f"但**数值全部是造出来的**（维度值带「示例」字样）。"
                      f"回答里必须声明这一点，绝对不可用于任何业务结论。")
